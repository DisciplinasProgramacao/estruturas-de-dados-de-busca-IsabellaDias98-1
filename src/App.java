...
public class App {
     ...
     * Deve haver uma linha para cada um dos produtos. Retorna uma árvore vazia
     * em caso de problemas com o arquivo.
     *
     * @param nomeArquivoDados Nome do arquivo de dados a ser aberto.
     * @return Uma árvore com os produtos carregados, ou vazia em caso de
     * problemas de leitura.
     */
    static <K> long lerProdutos(String nomeArquivoDados, Function<Produto, K> extratorDeChave, ABB<K, Produto> arvoreDestino) {
        ...
        try {
            ...
            for (int i = 0; i < numProdutos; i++) {
                ...
                arvoreDestino.inserir(chave, produto);
            }
            quantosProdutos = numProdutos;

        } catch (IOException excecaoArquivo) {
            System.out.println("Erro: " + excecaoArquivo);
        } finally {
            if (arquivo != null) {
                arquivo.close();
            }
     ...
     * código de produto informado pelo usuário, e o retorna. Em caso de não
     * encontrar o produto, retorna null
     */
    ...
    /**
     * Localiza um produto na árvore de produtos organizados por nome, a partir
     * do nome de produto informado pelo usuário, e o retorna. A busca não é
     * sensível ao caso. Em caso de não encontrar o produto, retorna null
     */
     ...
     * partir do código de produto informado pelo usuário, e o retorna. Em caso
     * de não encontrar o produto, retorna null
     */
    ...
    /**
     * Localiza e remove um produto na árvore de produtos organizados por nome,
     * a partir do nome de produto informado pelo usuário, e o retorna. A busca
     * não é sensível ao caso. Em caso de não encontrar o produto, retorna null
     */
    ...
    private static <K> void executarRecorte(ABB<K, Produto> arvore, String tipoChave, Function<String, K> parser) {
        cabecalho();
        System.out.println("Recorte de Produtos por " + tipoChave);

        if (arvore == null || arvore.vazia()) {
            System.out.println("A árvore de " + tipoChave + " está vazia.");
            return;
        }

        System.out.print("Digite o " + tipoChave + " inicial: ");
        String textoInicio = teclado.nextLine();
        
        System.out.print("Digite o " + tipoChave + " final: ");
        String textoFim = teclado.nextLine();

        try {
            K inicio = parser.apply(textoInicio);
            K fim = parser.apply(textoFim);
            
            System.out.println("Processando...");
            Lista<Produto> resultado = arvore.recortar(inicio, fim);
            
            System.out.println("\n--- Produtos no intervalo [" + inicio + " - " + fim + "] ---");
    ...
    private static void recortarProdutosNome(ABB<String, Produto> produtosCadastrados) {
        executarRecorte(produtosCadastrados, "Nome", (s) -> s);
    }

    private static void recortarProdutosId(ABB<Integer, Produto> produtosCadastrados) {
        executarRecorte(produtosCadastrados, "ID", Integer::parseInt);
    }

    public static void main(String[] args) {
        teclado = new Scanner(System.in, Charset.forName("UTF-8"));
        nomeArquivoDados = "C:/Users/isabe/OneDrive/Documents/AEDs II/Estruturas de Dados de Busca/estruturas-de-dados-de-busca-IsabellaDias98-1/produtos.txt";
        ...
        do {
            ...
            switch (opcao) {
                case 1 -> {
                    System.out.println("--- Carregando por Nome ---");

                    produtosCadastradosPorNome = new ABB<>();
                    long tempoABB = lerProdutos(nomeArquivoDados, (p -> p.descricao), produtosCadastradosPorNome);
                    System.out.println("Tempo da ABB: " + tempoABB + " ms");

                    produtosBalanceadosPorNome = new AVL<>();
                    long tempoAVL = lerProdutos(nomeArquivoDados, (p -> p.descricao), produtosBalanceadosPorNome);
                    System.out.println("Tempo da AVL: " + tempoAVL + " ms");
                }
                case 2 -> {
                    System.out.println("--- Carregando por ID ---");

                    produtosCadastradosPorId = new ABB<>();
                    long tempoABB = lerProdutos(nomeArquivoDados, (p -> p.idProduto), produtosCadastradosPorId);
                    System.out.println("Tempo da ABB: " + tempoABB + " ms");

                    produtosBalanceadosPorId = new AVL<>();
                    long tempoAVL = lerProdutos(nomeArquivoDados, (p -> p.idProduto), produtosBalanceadosPorId);
                    System.out.println("Tempo da AVL: " + tempoAVL + " ms");
                }
                case 3 ->
                    mostrarProduto(localizarProdutoNome(produtosCadastradosPorNome));
                case 4 -> {
                    System.out.println("Busca na ABB (ID):");
